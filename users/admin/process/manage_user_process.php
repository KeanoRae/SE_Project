<?php
    ...
    if(isset($_POST['delete'])){
        try{
            $sql = $db->prepare("DELETE FROM user WHERE id=:table_id");
            //bind
            $sql->bindParam(':table_id', $_POST['table_id']);
            if($sql->execute()){
                header('Location: manage_user.php');
            }           
        }
    ...
    if(isset($_POST['update'])){
        ...
        if(!in_array("",$var)){
            try{
                ...
                $getpassword = $db->prepare("SELECT password FROM user WHERE id=:table_id");
                //bind Param
                $getpassword->bindParam(':table_id', $_POST['table_id']);
                $getpassword->execute();
                if($row=$getpassword->fetch(PDO::FETCH_ASSOC)){
                    $oldpass = $row['password'];

                    //check if password match
                    if(password_verify($_POST['password'],$oldpass)){

                    }
                    //update query
                    $update = $db->prepare("UPDATE user SET username=:uname, password=:pw, first_name=:fname, last_name=:lname, role=:role WHERE id=:id");
                    //bind Param
                    $update->bindParam(':id', $_POST['table_id']);
    ...
    if(isset($GET['table_id'])){
        ...
        $display_sql->execute();
        if($inform=$display_sql->fetch(PDO::FETCH_ASSOC)){
